...
def main():
    ...
    output_root = Path("data/visualization/validated_data/transformer_usa300_only")
    output_root.mkdir(parents=True, exist_ok=True)

    prodigal_root = output_root / "prodigal"
    ...
    input_test = Path("data/raw/04_verified_sapis/01_seqs_with_host/individual_seqs/usa300-sapi5.fasta")
    logging.info("Predicting genes with Prodigal")
    prodigal = Prodigal(input_file=input_test, threads=num_threads, prodigal_output=prodigal_output)
    ...
    for protein in proteins:
        if protein.contig != current_contig:
            if current_contig is None:
                ...
                prediction(protein_sequence, hits, output_root, vocab_map, model, max_seq_length, chunk_size, satellite_size_filter, satellite_protein_size_filter, protein_stride, current_contig)
