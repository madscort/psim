...
## 2. Profile creation - de-novo
# 1. Collect proteins for each cluster in single fasta file -> fasta format
# 2. Align clusters with clustal omega -> stockholm format -> concat
# 3. Convert to msa -> convertmsa
# 4. Converto to profile -> msa2profile
...
def get_samples(all: bool, sampletable: Path=None) -> list[Path]:
    samples = []
...
def get_records_from_identifiers(records: List = None, identifiers: list = None):
    """
...
def convert_alignments(msa: Path, output: Path):
    ...
    cmd = ['mmseqs', 'msa2profile', msa_root / "msaDB", profile_root / "profileDB", '--match-mode', '1', '--cov', '0.9', '--qid', '0.5']
    subprocess_return = subprocess.run(cmd)
    if subprocess_return.returncode != 0:
...
if __name__ == '__main__':
    ...
    output = Path("data/processed/10_datasets/attachings_allDB")
